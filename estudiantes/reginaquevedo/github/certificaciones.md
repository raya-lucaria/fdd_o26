# Certificaciones de Git

Llena este archivo en **tu copia**, dentro de `estudiantes/<tu-login>/github/`.

## Quién soy

- Nombre: Regina Quevedo
- Usuario de GitHub: reginaquevedo
- Correo con el que entraste a DataCamp: rqueved2@itam.mx

## Introduction to Git

Se llena en la **primera** entrega.

Fecha en que lo terminaste: 16 de Septiembre, 2026

![Captura del curso Introduction to Git terminado](./introduccion-a-git.png)

## Intermediate Git

Se llena en la **segunda** entrega.

Fecha en que lo terminaste: 17 de septiembre

![Captura del curso Intermediate Git terminado](./git-intermedio.png)

## Una cosa que aprendiste y no sabías
Concrete mi conocimiento sobre merge, push y pull, como funcionan los remotes
Se llena en la **segunda** entrega, cuando ya hiciste los dos cursos.

Dos o tres líneas. Algo concreto que salió en alguno de los dos cursos y que
no habías visto en clase, o que en clase entendiste a medias y ahí se te
acomodó. Si sientes que no aprendiste nada nuevo, dilo y explica qué parte de
los cursos te pareció repetida.
Algo que aprendi que no vimos en clase fue el comando diff, que compara dos archivos o commits.
