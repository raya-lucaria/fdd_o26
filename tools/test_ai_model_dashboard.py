        ...
        "dashboard_models": [
            {
                ...
                "metrics": {
                    ...
                    "artifact_bytes": metric("FACT", 81_000_000_000, "byte", ("S_ART_A",)),
...
def test_training_scenarios_are_explicit_and_power_bases_are_not_combined():
    series = build_training_series(sample_ledger())
    ...
    assert (replacement.value, replacement.status, replacement.unit) == (
        Decimal("120000"), "SCENARIO", "USD"
    )
    assert set(power.source_ids) == {"S_A"}
    assert "S_COURSE_DESIGN" in replacement.source_ids
...
def test_inference_scenario_is_capacity_floor_not_sla():
    ...
    capacity = series["h100_capacity_equivalents"][0]
    power = series["accelerator_tdp_scenario"][0]
    capex = series["accelerator_capex_scenario"][0]
...
def test_inference_precision_selects_floor_and_rejects_unknown_precision():
    int4 = build_inference_series(
        sample_ledger(), CapacityScenario(precision="INT4")
...
def test_pareto_marks_safe_and_possible_frontiers_without_midpoints():
    ...
    assert result.possible_ids == ("A", "C")
